/**
 * The card-writing reference, as a note.
 *
 * The settings tab has always held this, which is the wrong place for it: you
 * read it while you are *writing* a card, and to reach it you had to leave the
 * canvas, open settings, read, close settings, and remember what it said. As a
 * note it sits in a pane beside the canvas and stays there.
 *
 * It is written into the vault rather than shipped as a file, so it belongs to
 * the vault afterwards: annotate it, move it, and the plugin opens your copy
...
- **Settings → Atlas** carries the same reference, always current with the build
- The example canvases, if you installed them: a cheat sheet, one card per
  feature, every way to put a picture on a card, and writing a theme
- \`Themes/_Template.css\` — every theme token, with a comment on each
`;

/** What the note is called, and how an existing one is recognised. */
export const REFERENCE_NAME = "How to write a card";
